    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            INSERT INTO #TempData(
                InventarioId, InventarioMarcaId, InventarioTipoId, InventarioNombreId, InventarioColorId, InventarioUnidadId,
                Cantidad, FechaCreacion, InventarioMarca, InventarioTipo, InventarioNombre, InventarioNombreAbreviatura,
                InventarioColor, InventarioColorAbreviatura, InventarioUnidad,
                InventarioMovimientoId, TipoMovimientoId, TipoMovimiento, CantidadTomada, Costo, FechaMovimiento, UsuarioId
            )
            SELECT
                i.InventarioId,
                i.InventarioMarcaId,
                ...
                mv.FechaCreacion,
                mv.UsuarioId
            FROM dbo.TblInventariosMovimientos mv (NOLOCK)
            INNER JOIN dbo.TblInventariosMovimientoTipos mvt (NOLOCK)
                ON mv.TipoMovimientoId = mvt.TipoMovimientoId
            INNER JOIN dbo.TblInventarios i (NOLOCK)
                ON i.InventarioId = mv.InventarioId
            ...
            IF (@elementoId = 0)
                SELECT * FROM #TempData ORDER BY FechaMovimiento DESC;
            ELSE
                SELECT * FROM #TempData WHERE InventarioId = @elementoId ORDER BY FechaMovimiento DESC;
