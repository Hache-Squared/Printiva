...
BEGIN TRY
    ...
    BEGIN
        ...
        THEN UPDATE SET Nombre = src.Nombre
        WHEN NOT MATCHED THEN
    ...
    -- Nota: Esta tabla no parece tener una columna ID identity propia, 
    -- se omite IDENTITY_INSERT a menos que falle.
    ------------------------------------------------------------------------------
...
BEGIN CATCH
    ...
    PRINT 'Error detectado. Se hizo ROLLBACK.';
    THROW;
END CATCH;